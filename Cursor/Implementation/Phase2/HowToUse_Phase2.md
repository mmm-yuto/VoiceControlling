...
2. `Create > Game > Creative Mode Settings` でアセットを作成（例：`CreativeModeSettings.asset`）  
   - ※ Unity が最新スクリプトをまだコンパイルしていない場合、`Game` メニューに表示されません。スクリプト保存後にコンパイル完了（エディタ右下の処理が終了）を待つか、Project ウィンドウでスクリプトを再インポートしてください。  
   - ※ それでも表示されない場合は `Assets > Create > Game >` 配下に「Creative」のサブメニューが追加されているか確認し、存在しない場合はスクリプトに `[CreateAssetMenu]` 属性が付いているかを確認してください。  
3. Inspector 設定例  
...
1. `Create > Game > Creative > Color Selection Settings`  
   - ※ メニューに表示されない場合は、`CreativeModeSettings` と同様にコンパイル完了を待つ／再インポートするか、`Assets > Create > Game > Creative` サブメニューに追加されているかを確認してください。  
2. Inspector 設定例  
...
1. `Create > Game > Creative > Save Settings`  
   - ※ `Creative` メニューが表示されない場合は、スクリプトのコンパイル完了や再インポートを確認してください。  
2. Inspector 設定例  
