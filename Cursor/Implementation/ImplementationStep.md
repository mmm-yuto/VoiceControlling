...
**基本方針**: シングルプレイモードを優先し、その次にオンラインマルチを実装。オフラインマルチは最も低い優先順位にする

1. **Phase 1: コアゲームシステム（プロトタイプ）**
   - `GameLogic/` - コアゲームロジック（塗りシステム、攻撃タイプ）
   - `Graphics/` - 基本的なエフェクト
...
### 🔴 Phase 1: コアゲームシステム（最優先）
1. PaintCanvas システム（Step 1.1.1）
2. 座標変換システム（Step 1.1.2）
3. インクエフェクト（Step 1.1.3）
4. 攻撃タイプ選択・管理システム（Step 1.2.1）
5. 各選択モードの実装（Step 1.2.2）
6. 攻撃タイプ判定（自動判定用、Step 1.2.3）
